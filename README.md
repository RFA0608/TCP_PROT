...
5. ☑️[70%] 암호화된 제어기 작동 (SEAL 설치 필요)

➡️[3단계 정체 이유] 

❗️제어기에서 발생하는 딜레이를 고려한 설계 필요

❗️python, cpp 가 아닌 golang 으로 구성된 제어기 코드 작성 필요
...
1. 윈도우에서 가상환경을 쓰지 않을 경우
  ...
  ```
  이 삽입되어 있는데 이것을 코드가 작동되는 파일 위치에 따라서 아래와 같이
  ```
  sys.path.append(r"../../python")
  ```
  으로 변경이 필요할 수 도 있습니다.
...
➤ 퀀서 장비를 돌리는 경우
TCP 서버의 주소를 모든 아이피를 허용할 수 있게 "0.0.0.0" 로 설정하고 포트 넘버 9999 를 윈도우 상에서 방화벽 해지를 해야합니다. 방화벽 해지 후 윈도우의 파워셀 커맨드창에 다음 명령어를 입력합니다.
```bash
ipconfig
```

WSL 의 설정을 변경하지 않았을 경우 네트워킹 모드가 Nat 으로 구성되어 있습니다. 따라서 TCP 클라이언트는 위 명령어를 쳐서 나오는 커맨드 결과 중 "이더넷 어댑터 vEthernet(WSL (Hyper-v firewall))의 inet 주소를 그대로 이용합니다. 포트는 서버와 같은 것을 이용하시면 됩니다.
...
3. **지연 있는 제어기의 ARX 버전 SEAL을 통한 암호화 된 제어기 with CPP**
   [퀀서 테스트 encrypted controller](https://youtu.be/brksf2zr6Z0) + [퀀서 테스트 original controller vs encrypted controller](https://youtu.be/4iE7Qzs6VXw)
